...
def find_sign_key(message, signature, dir="key_pairs/"):
    ...
    for file in os.listdir(dir):
        if not file.endswith(".pem"):
            continue  
        elif verify_message(message, signature, os.path.join(dir, file)):
...
def sign_message(message, priv_key_path, out_fname="signed_msg.txt"):
    ...
    msg = SHA256.new(message)
    # TODO: sign the message with the private RSA key using pkcs1_15
    signature = pkcs1_15.new(private_key).sign(msg)
...
def verify_message(message, signature, public_key_path):
    ...
    try:
        ...
        pkcs1_15.new(public_key).verify(msg, signature)
...
if __name__ == "__main__":
    while True:
        ...
        if option == "a":
            ...
            # TODO
            decrypt_key = find_decrypt_key(message)
            decrypt_message(message, decrypt_key, "sus_decrypted.txt")
        elif option == "c":
